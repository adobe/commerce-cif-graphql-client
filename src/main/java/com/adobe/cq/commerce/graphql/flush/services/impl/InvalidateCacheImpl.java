...
public class InvalidateCacheImpl implements InvalidateCacheService {
    ...
    public void invalidateCache(String path) {
        try (ResourceResolver resourceResolver = serviceUserService.getServiceUserResourceResolver(SERVICE_USER)) {
            ...
            if (resource != null) {
                String storePath = resource.getValueMap().get(PROPERTIES_STORE_PATH, String.class);
                ...
                if (commerceNode != null) {
                    ...
                    String storeView = commerceNode.hasProperty(PROPERTIES_STORE_VIEW) ? commerceNode.getProperty(PROPERTIES_STORE_VIEW)
                        .getString() : "default";

                    // Checks the graphql client exists
                    GraphqlClient client = getClient(graphqlClientId);

                    String[] invalidCacheEntries = resource.getValueMap().get(PROPERTIES_INVALID_CACHE_ENTRIES,
                        String[].class);
                    String[] listOfCacheToSearch = resource.getValueMap().get(PROPERTIES_LIST_OF_CACHE_TO_SEARCH,
                        String[].class);

                    String type = resource.getValueMap().get(PROPERTIES_TYPE, String.class);
                    String attribute = resource.getValueMap().get(PROPERTIES_ATTRIBUTE, String.class);
